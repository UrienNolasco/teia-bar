...
// Importações das funções do lado do servidor
...
const DeletarConsumo = () => {
  ...
  const handleOnClickSave = async () => {
    try {
      ...
      toast.error("Erro ao deletar consumos.");
    }
  };
    ...
    <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => setIsDialogOpen(open)}>
        <DialogContent className="w-[80%] rounded-2xl">
          ...
          <p>Você tem certeza que deseja deletar todos os consumos?</p>
          <p>Apenas faça isso após ter lançado todos os consumos.</p>
          <p>Esta ação não pode ser desfeita.</p>
          <DialogFooter className="mt-2">
            <Button className="bg-red-600" onClick={handleOnClickSave}>
              DELETAR
            </Button>
          </DialogFooter>
        </DialogContent>
